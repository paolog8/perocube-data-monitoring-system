...
-- Helper function to list queryable solar cells and pixels
CREATE OR REPLACE FUNCTION get_queryable_solar_cell_pixels()
RETURNS TABLE (
    solar_cell_name VARCHAR(255),
    pixel_identifier VARCHAR(255),
    first_connection_event_time TIMESTAMP WITH TIME ZONE,
    last_event_time TIMESTAMP WITH TIME ZONE,
    last_event_type_recorded VARCHAR(255)
)
AS $$
BEGIN
    -- This function lists all unique solar cell and pixel combinations
    -- that have at least one 'CONNECTED' event in the measurement_connection_event table.
    -- It also provides the timestamp of their first connection and last recorded event.
    RETURN QUERY
    WITH PixelFirstConnection AS (
        -- Find the first 'CONNECTED' event for each solar_cell_id and pixel
        SELECT
            mce.solar_cell_id,
            mce.pixel,
            MIN(mce.connection_datetime) AS first_connection_ts
        FROM
            measurement_connection_event mce
        WHERE
            mce.event_type = 'CONNECTED'
        GROUP BY
            mce.solar_cell_id, mce.pixel
    ),
    PixelLastEvent AS (
        -- Find the last event (any type) for each solar_cell_id and pixel
        SELECT
            mce.solar_cell_id,
            mce.pixel,
            mce.connection_datetime AS last_event_ts,
            mce.event_type AS last_event_type_val,
            ROW_NUMBER() OVER (PARTITION BY mce.solar_cell_id, mce.pixel ORDER BY mce.connection_datetime DESC) as rn
        FROM
            measurement_connection_event mce
        -- We need to join with PixelFirstConnection to ensure we only consider pixels that *have* had a 'CONNECTED' event.
        JOIN
            PixelFirstConnection pfc ON mce.solar_cell_id = pfc.solar_cell_id AND mce.pixel = pfc.pixel
    )
    SELECT
        pfc.solar_cell_id AS solar_cell_name,
        pfc.pixel AS pixel_identifier,
        pfc.first_connection_ts AS first_connection_event_time,
        ple.last_event_ts AS last_event_time,
        ple.last_event_type_val AS last_event_type_recorded
    FROM
        PixelFirstConnection pfc
    JOIN
        PixelLastEvent ple ON pfc.solar_cell_id = ple.solar_cell_id AND pfc.pixel = ple.pixel
    WHERE
        ple.rn = 1 -- Get only the latest event for each pixel
    ORDER BY
        solar_cell_name, pixel_identifier;
END;
$$ LANGUAGE plpgsql;
